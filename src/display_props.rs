...
fn display_childs(dir_properties: &Vec<DirectoryProperties>){
    for property in dir_properties{
        if property.child_flag(){
            println!("\nProperties of Children item of {} folder are as follows: ", property.location().expect("Err!! Retriving Location.").join(property.name().expect("Err!! Retriving file name.")).to_str().expect("Err!! While converting path to string."));
            match property.retrive_child_files() {
                Some(files) => {
                    println!("Children files are -");
